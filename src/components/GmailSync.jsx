      ...
      for(const{id}of fresh){
        ...
        try{
          ...
          for(const part of pdfParts){
            ...
            const fsize=part.body?.size||0;
            // Skip very small PDFs — T&C, welcome letters (< 50KB)
            if(fsize>0 && fsize<50000){
              log(`   ↳ ⏭ Skipping ${fname} (${Math.round(fsize/1024)}KB — too small to be a statement)`,"warn");
              continue;
            }
            // Skip non-statement filenames
            const fnLower=fname.toLowerCase();
            if(fnLower.includes("terms")||fnLower.includes("welcome")||fnLower.includes("insurance")||fnLower.includes("brochure")){
              log(`   ↳ ⏭ Skipping ${fname} — non-statement document`,"warn");
              continue;
            }
           ...
           while(!result && aiAttempts<3){
             ...
             try{
               ...
               if(pauseOnError){
                 doPause(`AI error on ${fname} — fix issue then Resume to retry`);
                 await waitIfPaused();
                 if(stopRef.current) break;
                 log(`   ↳ 🔄 Retrying same PDF (attempt ${aiAttempts+1}/3)...`);
               } else { break; }
             }
           }
           if(!result){ continue; }
           try{
            ...
            log(`   ↳ ❌ Post-processing error: ${postErr.message}`,"error");
          }
          }
        }catch(err){
