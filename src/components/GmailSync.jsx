...
async function callAIWithRetry(provider, apiKey, model, prompt, base64, mimeType, retries = 3, backupKey = null) {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (isRateLimit) {
        // Try backup key first before waiting
        if (!usedBackup && backupKey && backupKey !== currentKey) {
          console.warn(`Rate limit on primary key — switching to backup key...`);
          currentKey = backupKey;
          usedBackup = true;
          continue;
        }
        // Both keys exhausted — wait for retry time from error message
        const retryMatch = msg.match(/retry in (\d+(\.\d+)?)s/i);
        const waitSec = retryMatch ? Math.ceil(parseFloat(retryMatch[1])) + 2 : 60;
        console.warn(`Rate limited on both keys — waiting ${waitSec}s...`);
        await sleep(waitSec * 1000);
        // Reset to primary key after wait
        currentKey = apiKey;
        usedBackup = false;
        continue;
      }
...
export async function callGroq(apiKey, base64, mimeType = "image/jpeg", providerOverride = null, modelOverride = null, backupKey = null) {
  const provider = providerOverride || "groq";
  const model = modelOverride || AI_PROVIDERS[provider]?.defaultModel ||
    "meta-llama/llama-4-scout-17b-16e-instruct";

  // Pass 1: Extract
  const first = await callAIWithRetry(provider, apiKey, model, EXTRACTION_PROMPT, base64, mimeType, 3, backupKey);

  // Pass 2: Verify (always — cheap insurance against digit misreads)
  let result;
  try {
    const verified = await callAIWithRetry(provider, apiKey, model, VERIFY_PROMPT(first), base64, mimeType, 3, backupKey);
